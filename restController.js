...
exports.findById = (req,res)=>{
    Restaurant.find({restaurant_id: req.params.restId}).then((data)=>{
        res.status(200).send(data);
    }).catch((error) => {
        res.status(404).send({ message: "Error : ",error });
      })
...
exports.UpdatebyId = (req,res)=>{
    Restaurant.findOneAndUpdate( {restaurant_id: req.params.restId}, {$set: {address: req.body.address}}).then((data)=>{
        res.status(200).send(data);
    }).catch((error) => {
...
exports.deleteRest = (req, res) => {
    Restaurant.findOneAndDelete({restaurant_id: req.params.restId},{$exists: true}).then((data)=>{
        if(data){
...
exports.getGrades = (req, res) => {
    Restaurant.findById({_id : req.params._id}).then((data)=>{
...
//get unique cuisines
exports.cuisines = (req,res, err)=> {
    Restaurant.distinct("cuisine").then((data)=>{
        res.status(200).send(data)
        // console.log(err)
    }).catch((error)=>{
        res.status(404).status("Not found.")
...
exports.cuisineRest = (req, res) => {
    Restaurant.find({cuisine : req.params.cuisine}, {name:1}).then((data)=>{
        res.status(200).send(data)
    })
    .catch((error)=>{
        res.status(404).status("Not found.")
